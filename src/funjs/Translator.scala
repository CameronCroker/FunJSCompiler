...
// FIXME
            case WhereExp(e, List()) =>
               translateExpression(e) 
            case WhereExp(e, (Defn(IdnDef(idn,tipe), body))::ds) =>
                translateExpression(AppExp(FunExp(IdnDef(idn,tipe),WhereExp(e,ds)),body))

            case ObjExp(ds)=>
            val defs = ds.collect {case Defn (IdnDef(i,t), e) => (i -> e)}
                                           var map = Map[Identifier, Frame]()
                                           defs.foreach {case (i, e) => 
                                               map = map + (i -> translateExpression(e))
                                                  }
            List(IObj(map))

            case DeRefExp(IdnUse(i), name) =>
            List(IDeRef(i,name))
